...
class HeadlessVATSIMMonitor:
    ...
    def __init__(self):
        # Load configuration for display name
        self.config = load_config()
        
        # Get airport configuration for display purposes
        ...
        # Simplified application state (most state now handled by worker)
        self.monitoring = False
        self._shutting_down = False

        # Setup worker (now extends BaseMonitoringService with built-in state management)
        self.setup_worker()
    ...
    def setup_worker(self):
        ...
        # The worker now handles notifications through the base class
        # Just set up error callback for logging
    ...
    def test_pushover(self):
        """Test Pushover notification (delegate to worker)"""
        return self.worker.notification_manager.test_pushover()
...
def main():
    ...
    try:
        ...
        if monitor.worker.notification_manager.pushover_service:
